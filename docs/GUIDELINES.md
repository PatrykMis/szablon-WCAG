...
Zapisując kod źródłowy należy użyć znaczników pre oraz code z odpowiednią klasą                                biblioteki highlight.js [Zobacz wykaz wspieranych języków i klas.](https://github.com/highlightjs/highlight.js/blob/main/SUPPORTED_LANGUAGES.md)

Zapisując kod źródłowy z języka HTML należy zamienić znak < oraz > na `&lt;` oraz `&gt;`. Można posłużyć się do tego automatycznym  narzędziem, np. [HTML Encoder](https://www.opinionatedgeek.com/codecs/htmlencoder).
